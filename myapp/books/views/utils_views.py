...
from translation.services import TranslationService, ValidationError as TranslationValidationError, APIError, RateLimitError
...
class ChapterAnalyzeView(LoginRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        ...
        return JsonResponse(
            {"success": True, "message": "Chapter analysis not yet implemented", "chapter": chapter.id}
        )
...
class TaskStatusView(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if task_type == "translation":
            try:
                job = get_object_or_404(TranslationJob, id=task_id, created_by=request.user)
                ...
                if is_success:
                    ...
                    if translated_chapter:
                        response_data["redirect_url"] = reverse("books_admin:chapter_detail", kwargs={"pk": translated_chapter.id})
                        response_data["message"] = f"Translation to {job.target_language.name} completed successfully!"
                    else:
                        response_data["message"] = "Translation completed but chapter not found."
                        
                elif is_failure:
                    response_data["message"] = job.error_message or "Translation failed"
                    
                elif is_processing:
                    response_data["message"] = f"Translating to {job.target_language.name}..."
                    
                else:  # pending
                    response_data["message"] = f"Translation to {job.target_language.name} is queued"
...
class ChapterTranslationView(LoginRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if Chapter.objects.filter(chaptermaster=chapter.chaptermaster, book__language=target_language).exists():
                return JsonResponse({
                    "success": False, 
            ...
            return JsonResponse({
                ...
                "status_url": reverse("books_admin:task_status") + f"?task_id={translation_job.id}&task_type=translation"
            })
            ...
            return JsonResponse({
                "success": False, 
                "error": f"Failed to queue translation: {str(e)}"
            })
